...
export default function TeacherLoginPage() {
  ...
  useEffect(() => {
    const fetchAppSettings = async () => {
      setIsLoading(true);
      if (!firestore) {
        console.warn("Firestore instance not available in TeacherLoginPage. Using defaults.");
      ...
      try {
        ...
        if (settingsDocSnap.exists()) {
          ...
          console.warn("App settings document not found for TeacherLoginPage. Using defaults.");
          setAppName('EES Education');
          setLogoUrl(null);
        }
      } catch (error) {
        console.error("Error fetching app settings for teacher login page:", error);
        setAppName('EES Education');
        setLogoUrl(null);
      }
      setIsLoading(false);
