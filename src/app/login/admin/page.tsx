...
export default function AdminLoginPage() {
  ...
  useEffect(() => {
    const fetchAppSettings = async () => {
      setIsLoading(true);
      if (!firestore) {
        console.warn("Firestore instance not available in AdminLoginPage. Using defaults.");
      ...
      try {
        ...
        if (settingsDocSnap.exists()) {
          ...
          console.warn("App settings document not found for AdminLoginPage. Using defaults.");
          setAppName('EES Education');
          setLogoUrl(null);
        }
      } catch (error) {
        console.error("Error fetching app settings for admin login page:", error);
        setAppName('EES Education');
        setLogoUrl(null);
      }
      setIsLoading(false);
