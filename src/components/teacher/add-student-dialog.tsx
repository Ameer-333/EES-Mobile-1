...
import { getStudentProfilesCollectionPath, getUserDocPath, USERS_COLLECTION } from '@/lib/firestore-paths'; // Updated import
...
export function AddStudentDialog({ isOpen, onOpenChange, onStudentAdded }: AddStudentDialogProps) {
  ...
  async function onSubmit(values: AddStudentFormValues) {
    ...
    // Use the helper function to get the path to the 'profiles' subcollection for this student's class
    ...
    try {
      ...
      // 2. Prepare student document for the class-specific 'profiles' subcollection
      const studentProfileDataForDb: Omit<Student, 'id'> = { // 'id' will be auto-generated by Firestore in the subcollection
        authUid: authUid,
        name: values.name,
      ...
      // This `addDoc` call creates the 'profiles' subcollection under `student_data_by_class/<classId>/` if it doesn't exist,
      // and adds the new student's profile document into it.
      const studentDocRef = await addDoc(collection(firestore, studentProfilesCollectionPath), studentProfileDataForDb);
      const studentProfileId = studentDocRef.id; // This is the ID of the document in the 'profiles' subcollection
      ...
      const userDocData: Omit<ManagedUser, 'id' | 'lastLogin' | 'assignments'> = {
        ...
        status: 'Active',
        classId: values.classId,
        studentProfileId: studentProfileId, // Store the ID from the 'profiles' subcollection
      };
      await setDoc(doc(firestore, getUserDocPath(authUid)), userDocData); // Use helper for user doc path
    ...
    <Dialog open={isOpen} onOpenChange={(open) => {
      ...
      <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Enter student details. Login email & password will be auto-generated. Student profile will be stored in: student_data_by_class/<strong>{form.getValues('classId') || '{ClassID}'}</strong>/profiles/
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
